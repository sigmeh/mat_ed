/* 
	Renamed GLmol.js to zGLmol.js to ensure this script loads after three.js script (from same directory)
	
	Lines ~1618: 
		Original data source is " $('#' + this.id + '_src').val() " , corresponding to the 
		textarea in which data is embedded (i.e., from 'embedding-examplesEN.html' in GLmol directory)
		
		The original command is: 
			this.loadMoleculeStr(repressZoom, $('#' + this.id + '_src').val());
		
		
		The data source was changed to file_content of current_material, via: 
				
				var mat_id = Session.get('current_material_id');
				var file_content = mats.findOne({_id: mat_id}).file_content;
				this.loadMoleculeStr(repressZoom, file_content );
...
//
//		GLmol.prototype.drawAtomsAsSphere 
//
